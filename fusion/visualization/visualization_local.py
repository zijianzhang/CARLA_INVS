...
if __name__ == "__main__":
    ...
    for vehicle_id in vehicle_list:
        for frame_id in frame_id_list:
            ...
            gt_file = root_path + '/' + vehicle_id + '/training/label_2/' + frame_id
            pcd_file = root_path + '/' + vehicle_id + '/training/velodyne/' + frame_id[:-3] + 'bin'
            calib_file = root_path + '/' + vehicle_id + '/training/calib/' + frame_id
            ...
            baseline_file = root_path + '/' + vehicle_id + '/training/label_baseline/' + frame_id
            baseline_label = get_label(baseline_file)
            baseline_bbox = get_bboxes(baseline_label,calib,[0.,1.,0.])

            proposed_file = root_path + '/' + vehicle_id + '/training/label_proposed/' + frame_id
            proposed_label = get_label(proposed_file)
            proposed_bbox = get_bboxes(proposed_label,calib,[0.,0.,1.])

            mesh = o3d.geometry.TriangleMesh.create_coordinate_frame(size=10)
            # geometry_list = [mesh] + pcd + gt_bbox# + pretrain_fusion_bbox
            geometry_list = pcd + gt_bbox + baseline_bbox + proposed_bbox + [mesh]
            # save_view_point(pcd[0],'nb.json')
            # print(figure_file)
            custom_draw_geometry(vis,geometry_list,figure_file,recording=True)

#  python3 Visualization/Visualization.py ./data/record2020_1027_1957 715 39336