...
from avg_power import average_power
import pdb
import copy
import time
from sys import argv

if int(argv[1]) == 0: # (adandon) fastest with most NaN
...
if int(argv[1]) == 1: #slow with less NaN
    sp.run(['bash', '-c', 'cd tools; python3 train.py --cfg_file cfgs/train1_new_slow.yaml --batch_size 1 --workers 0 --epochs 2 --pretrained_model ../model/model_avg.pth']) #return model_0
    sp.run(['bash', '-c', 'cd tools; python3 train.py --cfg_file cfgs/train2_new_slow.yaml --batch_size 1 --workers 0 --epochs 2 --pretrained_model ../model/model_avg.pth']) #return model_1
    sp.run(['bash', '-c', 'cd tools; python3 train.py --cfg_file cfgs/train4_new_slow.yaml --batch_size 1 --workers 0 --epochs 2 --pretrained_model ../model/model_avg.pth']) #return model_1
    sp.run(['bash', '-c', 'cd tools; python3 train.py --cfg_file cfgs/train5_new_slow.yaml --batch_size 1 --workers 0 --epochs 2 --pretrained_model ../model/model_avg.pth']) #return model_1
...
params_avg = avgrage_power(w_locals)
